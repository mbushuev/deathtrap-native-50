...
int main() {
  ...
  const ImmersiveFirstPersonPose forward =
      BuildImmersiveFirstPersonPose(root, 0.0, 0.0, 485, 180);
  Require(forward.valid, "forward pose must be valid");
  Require(forward.eye == std::array<int32_t, 3>{100, 685, 120},
          "yaw zero eye must be 180 units toward negative Z");
  ...
  const ImmersiveFirstPersonPose right =
      BuildImmersiveFirstPersonPose(root, -kPi / 2.0, 0.0, 485, 180);
  Require(right.eye == std::array<int32_t, 3>{280, 685, 300},
  ...
  const ImmersiveFirstPersonPose up =
      BuildImmersiveFirstPersonPose(root, 0.0, -kPi / 6.0, 485, 180);
  ...
  const ImmersiveFirstPersonPose invalid =
      BuildImmersiveFirstPersonPose(root, 0.0, 0.0, 0, 180);
  Require(!invalid.valid, "zero eye height must fail closed");
