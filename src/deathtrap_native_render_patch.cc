...
// 0x2E800 produces the collision-resolved endpoint inside 0x2E950. The latter
// subsequently limits the endpoint to 0x6E world units per source tick. That
// damping is useful for ordinary retail camera motion, but makes a spring arm
// visibly enter props before it is pulled out in several discrete steps. The
// optional hook below bypasses the limiter only for an aligned inward
// collision generated by our own request.
constexpr uintptr_t kCameraCollisionResolveRva = 0x0002E800u;
...
std::atomic<bool> g_camera_collision_raw_hooks_installed{false};
...
// Camera-centre distance and collision-volume radius are intentionally
// different quantities. A 96-unit swept sphere may need to collapse almost
// to the player pivot in a tight corner; clamping its centre to the sphere
// radius allowed the centre to cross a one-sided prop and expose the void
// behind it. Keep only a small non-zero centre offset for near-plane safety.
constexpr double kThirdPersonMinimumCameraDistance = 16.0;
...
using CameraCollisionResolveFn = void(__cdecl*)(
...
CameraCollisionResolveFn g_original_camera_collision_resolve = nullptr;
...
struct RawCameraCollisionCapture {
  ...
  std::array<int32_t, 3> raw{};
  std::array<int32_t, 3> authoritative{};
};
...
bool BuildThirdPersonOrbitPosition(void* controller,
  ...
  // A drawable node may be absent from one captured snapshot while crossing
  // a room boundary or refreshing its cache. Treat two adjacent clear samples
  // as part of the same contact manifold. Otherwise a one-tick miss expands
  // the arm by 48 units and the same mesh contracts it on the following tick,
  // producing the visible 96->144->96 shake seen in v0.0.72 logs.
...
bool ClipThirdPersonOrbitAgainstSceneObjects(
  ...
  constexpr double kMinimumCameraDistance =
      kThirdPersonMinimumCameraDistance;
  ...
  if (nearest_initial_overlap && unresolved_overlap) {
...
void __cdecl HookCameraCollisionResolve(
    void* controller, const int32_t* averaged_position,
    const int32_t* camera_basis, int32_t* resolved_position) {
  if (g_original_camera_collision_resolve) {
    g_original_camera_collision_resolve(controller, averaged_position,
                                        camera_basis, resolved_position);
  }

  auto& capture = g_raw_camera_collision_capture;
  if (!capture.armed || capture.controller != controller ||
      !resolved_position) {
    return;
  }

  std::array<int32_t, 3> resolved{};
  if (!SafeRead(resolved_position, resolved.data(), sizeof(resolved))) {
    return;
  }
  capture.raw = resolved;
  capture.raw_valid = true;

  const double requested_x = static_cast<double>(
      capture.requested[0] - capture.player[0]);
  const double requested_y = static_cast<double>(
      capture.requested[1] - capture.player[1]);
  const double requested_z = static_cast<double>(
      capture.requested[2] - capture.player[2]);
  const double resolved_x = static_cast<double>(
      resolved[0] - capture.player[0]);
  const double resolved_y = static_cast<double>(
      resolved[1] - capture.player[1]);
  const double resolved_z = static_cast<double>(
      resolved[2] - capture.player[2]);
  ...
  // native publisher then emits the safe point immediately, while free orbit
  // motion and outward spring-arm recovery retain their existing damping.
  if (!SafeWrite(reinterpret_cast<uint8_t*>(controller) +
...
void UpdateThirdPersonCollisionRadius(void* controller) {
  ...
  if (g_raw_camera_collision_capture.controller == controller &&
    ...
    const double mesh_radius = std::clamp(
        g_raw_camera_collision_capture.render_mesh_radius,
        kThirdPersonMinimumCameraDistance,
        requested_distance);
  ...
  // The retail resolver is retained inside the game's configure call as a
  // same-frame room/BSP safety net, but it must never feed persistent modern
  // spring-arm state.  Its output contains the old camera's follow damping,
  // and v0.0.71 logs showed 63 false contractions while simply running.  The
  // exact render-mesh swept sphere above is now the sole owner of arm length.
  g_third_person_orbit_state.collision_confirmation_ticks = 0;
  g_third_person_orbit_state.collision_candidate_radius = 0.0;
  g_third_person_orbit_state.collision_candidate_ticks = 0;
  g_third_person_orbit_state.collision_clear_ticks =
      std::min(g_third_person_orbit_state.collision_clear_ticks + 1u, 120u);
}
...
void __cdecl HookMode3Camera(void* controller) {
  ...
  g_raw_camera_collision_capture.armed =
      g_camera_collision_raw_hooks_installed.load(std::memory_order_acquire);
  ...
  if (unresolved_object_overlap) {
    // The sweep began with its collision volume already touching geometry.
    // A translated previous endpoint or the retail camera may be on the far
    // side of that one-sided mesh, so neither is a safe fallback. The sweep
    // has deliberately returned the near-pivot point on the player's side;
    // publish it immediately and keep it out of the last-safe cache.
    collision_target[1] = std::max(collision_target[1], minimum_camera_y);
    if (g_debug_log) {
      AppendNativeLog(
          "camera_overlap_collapse target=%d/%d/%d player=%d/%d/%d",
...
bool InstallDeathtrapNativeRenderHooks() {
  ...
  if (g_third_person_orbit_enabled) {
    ...
    void* const collision_resolve_target =
        g_dungeon_base + kCameraCollisionResolveRva;
    const MH_STATUS create_collision_resolve = MH_CreateHook(
        collision_resolve_target,
        reinterpret_cast<void*>(&HookCameraCollisionResolve),
        reinterpret_cast<void**>(&g_original_camera_collision_resolve));
    if (create_collision_resolve == MH_OK ||
        create_collision_resolve == MH_ERROR_ALREADY_CREATED) {
      const MH_STATUS enable_collision_resolve =
          MH_EnableHook(collision_resolve_target);
      if (enable_collision_resolve == MH_OK ||
          enable_collision_resolve == MH_ERROR_ENABLED) {
        g_camera_collision_raw_hooks_installed.store(
            true, std::memory_order_release);
        AppendNativeLog(
            "camera_collision raw_hook=active rva=%08llX "
            "native_damping_bypass=aligned_inward_only",
            static_cast<unsigned long long>(kCameraCollisionResolveRva));
      } else {
        AppendNativeLog(
            "camera_collision raw_hook=enable_failed status=%d fallback=1",
            static_cast<int>(enable_collision_resolve));
      }
    } else {
      AppendNativeLog(
          "camera_collision raw_hook=create_failed status=%d fallback=1",
          static_cast<int>(create_collision_resolve));
    }
    ...
    if (create_camera == MH_OK ||
      ...
      if (enable_camera == MH_OK || enable_camera == MH_ERROR_ENABLED) {
        ...
        AppendNativeLog(
            ...
            g_camera_collision_raw_hooks_installed.load(
